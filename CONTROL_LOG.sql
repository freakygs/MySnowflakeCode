...
CREATE or replace PROCEDURE SP_EDL_PROC_CNTL_LOG
		(OPCODE NUMBER(38,0), 
...
BEGIN
  CASE
    WHEN :OPCODE=0 THEN
        ...
        INSERT INTO EDLDEVUSER.EDL_PROC_CONTROL_LOG
        ...
        call EDLDEVUSER.SP_WH_ALLOCATE(:FUNCTION_GROUP,:BATCH_GROUP);
    
    WHEN :OPCODE=1 THEN
	call EDLDEVUSER.SP_WH_ALLOCATE('DEFAULT','DEFAULT'); 
        UPDATE EDLDEVUSER.EDL_PROC_CONTROL_LOG SET EXECUTION_END_TS=CURRENT_TIMESTAMP(0),
            ...
            EXECUTION_START_TS=(SELECT MAX(EXECUTION_START_TS) FROM EDLDEVUSER.EDL_PROC_CONTROL_LOG  WHERE 
								SNC_NAMESPACE=:SNC_NAMESPACE AND SNC_TABLENAME=:SNC_TABLENAME);
            
            INSERT INTO EDLDEVUSER.EDL_PROC_EXEC_LOG
    ...
    ELSE
	call EDLDEVUSER.SP_WH_ALLOCATE('DEFAULT','DEFAULT'); 
        UPDATE EDLDEVUSER.EDL_PROC_CONTROL_LOG SET EXECUTION_STATUS_CD='F',
            PROC_OUTPUT=:PROC_OUTPUT WHERE 
            BUS_FUNCTION_GROUP=:FUNCTION_GROUP AND
            BATCH_GROUP=:BATCH_GROUP AND
            SNC_NAMESPACE=:SNC_NAMESPACE AND
